...
export const actions = {
  ...
  updateProfile: async ({ request, locals: { supabase, safeGetSession } }) => {
    console.log("updateProfile ACTION fired!!!, ", request)
    const { session } = await safeGetSession()
    if (!session) {
      redirect(303, "/login")
    }
    ...
    if (!schoolName) {
      validationError =
        "School or Family name is recommended. If this is a single child or personal hobby, please put your name."
      errorFields.push("schoolName")
    } else if (schoolName.length > fieldMaxTextLength) {
      validationError = `School name must be less than ${fieldMaxTextLength} characters`
      errorFields.push("schoolName")
    }
